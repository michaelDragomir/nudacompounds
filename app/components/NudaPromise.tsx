			...
			<div className='mx-auto max-w-6xl px-6'>
				...
				<div className='grid gap-6 lg:grid-cols-3'>
					{PROMISE_ITEMS.map(({ badge, title, value, body }) => (
						<div
							key={title}
							className='rounded-2xl border border-amber/30 bg-offwhite p-8 text-center shadow-xl'
						>
							<span className='inline-flex items-center gap-2 rounded-full border border-amber/40 px-3 py-1 text-xs font-bold uppercase tracking-wide text-amber-dark'>
								<span className='h-1.5 w-1.5 rounded-full bg-amber' />
								{badge}
							</span>

							<h3 className='mt-5 text-2xl font-bold leading-snug text-navy'>
								{title}
							</h3>

							{value && (
								<p className='mt-3 text-sm text-warmgray line-through'>
									{value}
								</p>
							)}

							<p className='mt-1 text-4xl font-bold tracking-wide text-amber-dark'>
								FREE
							</p>

							<span className='mx-auto mt-4 block h-px w-10 bg-navy/10' />
